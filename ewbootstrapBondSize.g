...
@ Step 1: ------Name of the Gauss data set.----------------------------------@
    ...
    closeall f1;
    open f1=^dataset;
    aalldta=readr(f1,rowsf(f1));                   @  alldta = selif(alldta,alldta[.,3].>=1992 .or alldta[.,3].<=1995);       @
    vnames = getname(dataset);
    aalldta = selif(aalldta,aalldta[.,3].>=92 .and aalldta[.,3].<=95);
@ Step 2: ------Name of the output file.-------------------------------------@


    output file = ewbootstrapBondSize.out reset;
...
@ Step 4: ------Names of the perfectly measured variables.-------------------@
    ...
    let zname = intercep cf k assets;
...
do while bootloop <= nboot;

if floor(bootloop/10)==(bootloop/10);
locate 3,3;
output off;
"I am bootstrap trial ";; bootloop;
output on;
endif;
@----------------The saver for the identifiers.----@
   ...
   do until year > nyr;
    ...
    @ --------- Read in the data and define the variables. ----------@
      ...
      dta=selif(alldta,(alldta[.,3].==(fyear+year-1)));
      if bootloop > 0;
      dta = dta[bootpick,.];
      endif;
...
elseif estim==5;
 ...
 neq = 29;
    ...
    if estim > 1 and c[1,1]<bsave[1,year]; c[1,1] = bsave[1,year]; c[1,1] = meanc(rbsave[1,year]|bsave[1,year]); endif;
    if estim > 1 and c[1,1]>rbsave[1,year]; c[1,1] = rbsave[1,year]; c[1,1] = meanc(rbsave[1,year]|bsave[1,year]);endif;

  @
    if fe == 0 and estim == 2 and (year + fyear) == 1990; c[1,1] = 0.06; endif;


    if fe == 1 and estim == 2 and (year + fyear - 1) == 1971; c[1,1] = 0.06; endif;
    if fe == 1 and estim == 2 and (year + fyear - 1) == 1979; c[1,1] = 0.5; endif;
    if fe == 1 and estim == 2 and (year + fyear - 1) == 1990; c[1,1] = 0.206; endif;
    if fe == 1 and estim == 2 and (year + fyear - 1) == 1991; c[1,1] = 2.62; endif;
    if fe == 1 and estim == 2 and (year + fyear - 1) == 2006; c[1,1] = 0.06; endif;
    if fe == 1 and estim == 2 and (year + fyear - 1) == 2007; c[1,1] = 0.06; endif;

    if fe == 1 and estim == 3 and (year + fyear - 1) == 1979; c[1,1] = 0.06; endif;
    if fe == 1 and estim == 3 and (year + fyear - 1) == 1977; c[1,1] = 0.06; endif;
    if fe == 1 and estim == 3 and (year + fyear - 1) == 1989; c[1,1] = 0.06; endif;
    if fe == 1 and estim == 3 and (year + fyear - 1) == 1991; c[1,1] = 0.06; endif;
...
do until abs(dc) < tol;
   ...
   if bootloop == 0;                                                  @ The program jumps to the subroutine that defines the f vector @
      f = deff(c,Emom,0);
      effsave[1:neq,(estim-1)*nestim+year] =  f;
    else;
      f = deff(c,Emom,0) - effsave[1:neq,(estim-1)*nestim+year];
    endif;
  ...
  if panel == 1;
    ...
    if param == 1;
               if bootloop == 0;
                        ...
                        ccsave[3,estim] = cdfchic(cmdtest,3);
                        ccsave[4,estim] = stderror;
               else;
                        cpivot[estim,bootloop] = (theta-ccsave[1,estim])/stderror;
               endif;
    elseif param == 2;
               if bootloop == 0;
                        cpsave[1,estim] = theta;
                        cpsave[2,estim] = cmdtest;
                        cpsave[3,estim] = cdfchic(cmdtest,3);
                        cpsave[4,estim] = stderror;
               else;
                        ppivot[estim,bootloop] = (theta-cpsave[1,estim])/stderror;
               endif;
    ...
    elseif param == 3;
               if bootloop == 0;
                        ...
                        ctsave[3,estim] = cdfchic(cmdtest,3);
                        ctsave[4,estim] = stderror;
               else;
                        tpivot[estim,bootloop] = (theta-ctsave[1,estim])/stderror;
               endif;
    ...
    elseif param ==nz+4;
               if bootloop == 0;
                        ...
                        casave[3,estim] = cdfchic(cmdtest,3);
                        casave[4,estim] = stderror;
               else;
                        apivot[estim,bootloop] = (theta-casave[1,estim])/stderror;
               endif;
    ...
    if param == qq+3;
               ...
               if bootloop == 0;
                        ...
                        dpivot[(estim-1)*nz+qq,bootloop] = (theta-cdsave[(qq-1)*4+1,estim])/stderror;
               endif;
...
@ Compute t-ratios, etc.                                            @
    ...
    if bootloop == 0;
      ...
      spivot[estim,bootloop] = (theta-cdisave[1,estim])/stderror;
    endif;
    ...
    if param == 1;
      if bootloop == 0;
         ...
         bbsave[4,1] = stderror;
               ...
               bpivot[1,bootloop] = (theta-bbsave[1,1])/stderror;
      endif;
    elseif param == nz+2;
      if bootloop == 0;
         brsave[1,1] = theta;
         ...
         brsave[4,1] = stderror;
               ...
               rpivot[1,bootloop] = (theta-brsave[1,1])/stderror;
      endif;
    elseif param == nz+3;
      if bootloop == 0;
         bisave[1,1] = theta;
         ...
         bisave[4,1] = stderror;
                ...
                ipivot[1,bootloop] = (theta-bisave[1,1])/stderror;
       endif;
    endif;
    ...
    if param == qq+1;
      if bootloop == 0;
         ...
         bzsave[(qq-1)*4+4,1] = stderror;
               ...
               zpivot[qq,bootloop] = (theta-bzsave[(qq-1)*4+1,1])/stderror;
      endif;
    endif;
    endfor;
...
for jj(1, nyr, 1);
"& " zsave[(ee-1)*2+1,jj];;
for qq(((jj-1)*nestim+1), (nestim*jj), 1);
" & " dsave[(ee-1)*2+1,qq];; endfor; " \\" "\\";
"&(" zsave[ee*2,jj];;
for qq(((jj-1)*nestim+1), (nestim*jj), 1);
")&(" dsave[ee*2,qq];; endfor; ") \\" "\\";
endfor;
print;
...
for jj(1, nyr, 1);
for qq(((jj-1)*nestim+2), (nestim*jj), 1);
" & " chisave[1,qq];; endfor; " \\" "\\";
endfor;
print;print;
...
"& " isave[1,yy];; " & " bsave[1,yy];;
for tt(1, nz, 1); " & " zsave[(tt*2-1),yy];;
endfor; " & " Rsave[1,yy];; " \\" "\\";
"&(" isave[2,yy];; ")&(" bsave[2,yy];;
for tt(1, nz, 1); ")&(" zsave[(tt*2),yy];;
endfor; ")&(" Rsave[2,yy];; ") \\" "\\ [5pt]";
...
yy+fyear-1;; "& " asave[1,nestim*(yy-1)+qq];; " & " csave[1,nestim*(yy-1)+qq];;
for tt(1, nz, 1); " & " dsave[(tt*2-1),nestim*(yy-1)+qq];;
endfor; " & " psave[1,nestim*(yy-1)+qq];; " & " tsave[1,nestim*(yy-1)+qq];;
" & " chisave[1,nestim*(yy-1)+qq];;  " & "  nsave[1,yy];; " \\" "\\";
@yy;; @"         &(" asave[2,nestim*(yy-1)+qq];; ")&(" csave[2,nestim*(yy-1)+qq];;
for tt(1, nz, 1); ")&(" dsave[(tt*2),nestim*(yy-1)+qq];;
endfor; ")&(" psave[2,nestim*(yy-1)+qq];; ")&(" tsave[2,nestim*(yy-1)+qq];;
")&(" chisave[2,nestim*(yy-1)+qq];;   ")&           \\" "\\ [5pt]";
...
if bootloop == 0; cls; endif;
bootloop = bootloop + 1;
endo;

/* This business prints out the bootstrapped critical values for the CMD estimators. */


    locritvals = zeros(nestim + 1,5+nz);
    hicritvals = zeros(nestim + 1,5+nz);
    for estim(0,nestim,1);
    for ffff(0,4+nz,1);
           if ffff == 0;        if estim == 0;    pivotsort = 1;                          else; pivotsort = sortc(spivot[estim,.]',1); endif;
       elseif ffff == 1;        if estim == 0;    pivotsort = sortc(ipivot[1,.]',1);      else; pivotsort = sortc(apivot[estim,.]',1); endif;
       elseif ffff == 2;        if estim == 0;    pivotsort = sortc(bpivot[1,.]',1);      else; pivotsort = sortc(cpivot[estim,.]',1); endif;
       elseif ffff == 3;        if estim == 0;    pivotsort = sortc(rpivot[1,.]',1);      else; pivotsort = sortc(ppivot[estim,.]',1); endif;
       elseif ffff == 4;        if estim == 0;    pivotsort = zeros(nboot,1);             else; pivotsort = sortc(tpivot[estim,.]',1); endif;
       elseif ffff >  4;        if estim == 0;    pivotsort = sortc(zpivot[ffff-4,.]',1); else; pivotsort = sortc(dpivot[(estim-1)*nz+ffff-4,.]',1); endif;
       endif;
       locritvals[estim+1,ffff+1] = pivotsort[maxc(1|floor(rows(pivotsort)*0.025)),1];
       hicritvals[estim+1,ffff+1] = pivotsort[ceil(rows(pivotsort)*0.975),1];
    endfor;
    endfor;

   "Lower Critical Values";

   "      CMD Intercept:                                       ";; locritvals[.,2]';
   "      CMD Coefficient on the mismeasured regressor:        ";; locritvals[.,3]';
   for ffff(1,nz,1);
   "      CMD Coefficients on the perfectly measured regressor:";; locritvals[.,ffff+5]';
   endfor;
   "      CMD R-squared:                                       ";; locritvals[.,4]';
   "      CMD tau-squared:                                     ";; locritvals[.,5]';
   "      CMD SUM                                              ";; locritvals[.,1]';

   "Upper Critical Values";

   "      CMD Intercept:                                       ";; hicritvals[.,2]';
   "      CMD Coefficient on the mismeasured regressor:        ";; hicritvals[.,3]';
   for ffff(1,nz,1);
   "      CMD Coefficients on the perfectly measured regressor:";; hicritvals[.,ffff+5]';
   endfor;
   "      CMD R-squared:                                       ";; hicritvals[.,4]';
   "      CMD tau-squared:                                     ";; hicritvals[.,5]';
   "      CMD SUM                                              ";; hicritvals[.,1]';
...
"---------------------------------------------------------------------------";
   ...
   "& " bbsave[2,1];; for qq(1, nestim, 1); " & " ccsave[2,qq];; endfor; " \\" "\\";
   "& " bbsave[3,1];; for qq(1, nestim, 1); " & " ccsave[3,qq];; endfor; " \\" "\\";
   "& " ((bbsave[1,1]/bbsave[4,1]) .> hicritvals[1,1] .or (bbsave[1,1]/bbsave[4,1]) .< locritvals[1,1]);;
   ...
   "& " ((bzsave[(ee-1)*4+1,1]/bzsave[(ee-1)*4+4,1]) .> hicritvals[1,1] .or (bzsave[(ee-1)*4+1,1]/bzsave[(ee-1)*4+4,1]) .< locritvals[1,1]);;
   ...
   "& " brsave[2,1];; for qq(1, nestim, 1); " & " cpsave[2,qq];; endfor; " \\" "\\";
   "& " brsave[3,1];; for qq(1, nestim, 1); " & " cpsave[3,qq];; endfor; " \\" "\\";
   "& " ((brsave[1,1]/bisave[4,1]) .> hicritvals[1,1] .or (brsave[1,1]/bisave[4,1]) .< locritvals[1,1]);;
...
@-------------------------------------------------------@
@   This part makes the bootstrap adjustment            @
@-------------------------------------------------------@

if bootloop > 0;
   f = f - effsave[1:neq,(estim-1)*nestim+year]';
endif;
